            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {rankingData.map((data, index) => (
                  <tr key={data.id} className={index < 3 ? "bg-yellow-50" : ""}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-bold text-green-600">
                        +{data.annualSalary.toLocaleString()}円
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {(data.unitPrice / 10000).toFixed(0)}万円
                      </div>
                      <div className="text-sm text-gray-500">
                        還元率: {(data.returnRate * 100).toFixed(1)}%
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-blue-600">
                        {data.calcSalary.toLocaleString()}円
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-green-600">
                        {data.netAnnualSalary.toLocaleString()}円
                      </div>
                    </td>
